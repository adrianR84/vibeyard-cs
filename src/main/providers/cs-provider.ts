...
export class CsProvider implements CliProvider {
  readonly meta: CliProviderMeta = {
    id: 'cs',
    displayName: 'Claude Code CS',
    binaryName: 'cs',
    capabilities: {
      sessionResume: true,
      costTracking: true,
  ...
  resolveBinaryPath(): string {
    return resolveBinary('cs', binaryCache);
  }

  validatePrerequisites(): boolean {
    return validateBinaryExists('cs');
  }
  ...
  startConfigWatcher(win: BrowserWindow, projectPath: string): void {
    startConfigWatch(win, projectPath, 'cs');
  }
